...
DroneConn.prototype.connect = function() {
  ...
  var checker = setInterval(function() {
    if (!updated) {
      scope.uptime = (new Date().getTime() - scope.prevUptime.getTime()) / 1000;
      scope.prevUptime = new Date();
  ...
  req.on('error', function(data) {
    log.warn(scope.url, "disconnected, down for", scope.uptime, "seconds.");
    log.warn(data);
    scope.emit('noconn');
  });
