...
module.exports.sendEmailVerifyLink = function(link, email, cb) {
  ...
  try {
    ...
    var personalization = new helper.Personalization();
    var sub_link = new helper.Substitution("{{verify_link}}", link);
    personalization.addSubstitution(sub_link);
    personalization.addTo(to_email);

    mail.addPersonalization(personalization);
    mail.setTemplateId(config.mailer.templates.verify_email);
    mail.setFrom(from_email);
    mail.setSubject(subject);
