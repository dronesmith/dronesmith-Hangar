      ...
      async.waterfall([
        ...
        if (err) {
          ...
          if(err == "No user found."){
            ...
            res.status(400).json({error: config.errorMessages.password});
          } else {
...
exports.signUp = function(req, res, next) {
  async.waterfall([
    ...
    function (cbAsync) {
      dss.createUser(req.body.email, req.body.firstname, req.body.lastname, req.body.password, function(key, id, error){
        if(error){
          return cbAsync(error);
        } else {
          cbAsync(null, key, id);
