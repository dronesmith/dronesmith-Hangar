  ...
  setInterval(function() {
    ...
    async
      .each(droneMissions, function(missionMeta, cb) {
        ...
        dss.getAPI('/api/drone/position', missionMeta.apiInfo, (info, error) => {
          if (!error && info) {
            pos = info;
...
exports.start = function(req, res, next) {
  if (!req.body) {
    ...
    dss.getAPI('/api/drone/mode',
      req.session, (info, error) => {
        if (!error && info) {
          if (info != "Takeoff" && info != "Hold") {
            dss.postAPI('/api/drone/takeoff', {"altitude": 30},
